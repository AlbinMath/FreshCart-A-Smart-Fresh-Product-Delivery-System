...
// Function to check credentials (called at runtime, not import time)
const getEnvCreds = () => ({
  hasEnvCreds: !!(
...
console.log('hasEnvCreds:', creds.hasEnvCreds);
console.log('FIREBASE_PROJECT_ID:', creds.projectId ? 'SET' : 'NOT SET');
console.log('FIREBASE_CLIENT_EMAIL:', creds.clientEmail ? 'SET' : 'NOT SET');
console.log('FIREBASE_PRIVATE_KEY length:', creds.privateKey ? creds.privateKey.length : 'NOT SET');

if (!admin.apps.length) {
  if (creds.hasEnvCreds) {
