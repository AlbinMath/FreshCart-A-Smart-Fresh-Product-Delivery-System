...
function StoreRegister() {
  ...
  const handleInputChange = async (e) => {
    ...
    if (name === 'verificationFile') {
      setFormData(prev => ({ ...prev, verificationFile: files && files[0] ? files[0] : null }));
      return;
    }
  ...
  const handleRegister = async (e) => {
    ...
    if (!formData.sellerCategory) {
      return setError("Please select the category you sell (e.g., fruits, vegetables, seafood, meat, other)");
    }

    if (!businessLicenseValidation.isValid) {
      return setError(businessLicenseValidation.message);
    }

    // Prepare seller fields
    setError("");
    setLoading(true);
    ...
    // If already logged in, upgrade directly without password
    if (currentUser) {
      if (formData.email && currentUser.email && formData.email.toLowerCase() !== currentUser.email.toLowerCase()) {
        setError(`You're signed in as ${currentUser.email}. Please use that email or log out and try again.`);
        setLoading(false);
        return;
      }
      try {
        await upgradeToSeller(additionalInfo);
        setError("");
        setMessage("Seller account ready! If newly registered, please verify your email.");
        setTimeout(() => { navigate("/seller", { replace: true }); }, 1500);
      } catch (e) {
        setError("Failed to upgrade account: " + e.message);
      }
      setLoading(false);
      return;
    }

    // Not logged in: if email already registered, ask user to sign in first
    try {
      const isAvailable = await checkEmailAvailable(formData.email);
      if (!isAvailable) {
        setError("This email is already registered. Please sign in first, then submit this form to upgrade your account.");
        setLoading(false);
        return;
      }
    } catch (_) {}
    ...
    try {
      try {
        await signup(formData.email, formData.password, formData.name, "seller", additionalInfo);
      } catch (err) {
        const msg = String(err?.message || '').toLowerCase();
        // If backend says already in use, avoid password-based login to prevent invalid-credential
      ...
      <div className="max-w-md mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600">Register as a Store Owner</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <h2 className="text-2xl font-bold mb-6 text-center text-gray-900">Store Registration</h2>
          ...
          <div className="flex flex-wrap gap-2 bg-gray-100 p-2 rounded-xl mb-6">
            {[
              ...
              const active = id === "seller"; // This page is the Seller registration
              return (
                <button
                  key={id}
                  type="button"
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Store Owner Name</label>
              <input
                type="text"
                name="name"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">What category do you sell?</label>
              <select
                name="sellerCategory"
              ...
              <p className="mt-1 text-xs text-gray-500">This can be changed later in Profile, subject to admin review.</p>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Business Proof (optional)</label>
              <input
                type="file"
              ...
              {formData.verificationFile && (
                <p className="mt-1 text-xs text-gray-600">Selected: {formData.verificationFile.name}</p>
              )}
            </div>
            ...
            >
              {loading ? "Creating Store Account..." : "Register Store"}
            </button>
